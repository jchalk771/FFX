...

{
  "Building/Commercial/Company/amman05": {
    "Pageflow": [
      {
        "preScript": "",
        "duplicateRecordType": "Building/Commercial/xxx/yyy",
        "duplicateRecordTypeStatus":"",
        "requiredRecordType": "Building/Commercial/Company/amman05",
        "requiredRecordTypeStatus":"",
        "cancelSubmissionFields": [
          {
            "OwnerFullName": "Yazan S Barghouth",
            "MailZip": "12345"
          }
        ],
        "postScript": ""
...
- For duplicateRecordType and requiredRecordType, check if exist with status required in PDF, status was not mentioned in JSON, 2 properties were added:
	- duplicateRecordTypeStatus: empty means ignore status check
	- requiredRecordTypeStatus: empty means ignore status check
 * 
 */
...
function validateOwner(rules) {
	...
	if (rules.duplicateRecordType != null && rules.duplicateRecordType != "") {
		var capModel = getSearchCapModel(rules.duplicateRecordType, owner, rules.duplicateRecordTypeStatus);
		var capIDList = aa.cap.getCapIDListByCapModel(capModel).getOutput();

		if (capIDList != null && capIDList.length > 0) {
			return "Owner already have records of type: " + rules.duplicateRecordType + " with status: " + rules.duplicateRecordTypeStatus;
		}//apps count > 0
	}//chk duplicateRecordType

	//check requiredRecordType
	if (rules.requiredRecordType != null && rules.requiredRecordType != "") {
		var capModel = getSearchCapModel(rules.requiredRecordType, owner, rules.requiredRecordTypeStatus);
		var capIDList = aa.cap.getCapIDListByCapModel(capModel).getOutput();
		if (capIDList == null || capIDList.length == 0) {
			return "Owner should have records of type: " + rules.requiredRecordType + " with status: " + rules.requiredRecordTypeStatus;
		}//apps count == 0
...
function validateCancelFields(rules, refOwnerModel) {
	...
	for (f in rules.cancelSubmissionFields) {
		var filedsArray = rules.cancelSubmissionFields[f];
