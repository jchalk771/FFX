...
try {
	...
	for (s in settingsArray) {
		...
		// check if required rules match record data
...
function createAlert(rules) {
	...
	var alertUser = null;
	if (rules.alertAssign == "Workflow") {
		toUsersArray = getActiveTaskAssignedStaff(capId);
	} else if (rules.alertAssign == "Record") {
	...
	} else if (rules.alertAssign == "Manual") {
		if (rules.alertDepartment != null && rules.alertDepartment != "") {
			var users = aa.people.getSysUserListByDepartmentName(rules.alertDepartment).getOutput();
			for (u in users) {
				toUsersArray.push(users[u].getUserID());
			}//for all users in dept
		} else {
			if (rules.alertUser != null && rules.alertUser != "") {
				toUsersArray.push(rules.alertUser);
			}
		}
	} else {
		logDebug("**WARN invalid alertAssign value: " + rules.alertAssign);
		return false;
	}
	...
	return sendAlert(toUsersArray, rules.alertName, emailParameters);
}
