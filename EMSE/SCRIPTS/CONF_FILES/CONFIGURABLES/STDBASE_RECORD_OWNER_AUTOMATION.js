    ...
    "ApplicationSpecificInfoUpdateAfter": [
      {
        "preScript": "",
        "copyGISData": {
	          "BPTALM|Parcel_Owner|OwnerName": "OwnerFullName",
	          "BPTALM|Parcel_Owner|OwnerAddress": "Address1"
	        },
	    "mappingIdField":"FID",
...
function updateOwnerAndConditions(rules, recordCapId) {
	...
	if (parcelNumber != null) {
		updateOwnerFromGIS(ownerScriptModel, rules.copyGISData, rules.mappingIdField, parcelNumber);
	}

	if (rules.updateRecordOwnerFromRef != null && rules.updateRecordOwnerFromRef.equalsIgnoreCase("Y")) {
		if (ownerScriptModel == null) {
			//no owner with same refIdon cap, no need to check retainHistory
			createCapOwnerFromRefOwner(recordCapId, tmpRefOwner);
		} else {
			if (rules.retainHistory != null && rules.retainHistory.equalsIgnoreCase("Y")) {
		...
		if (rules.updateReferenceOwnerFromRec != null && rules.updateReferenceOwnerFromRec.equalsIgnoreCase("Y")) {
			updateRefOwnerFromOwner(owner, ownerScriptModel);
		}//updateReferenceOwnerFromRec
	}

	if (rules.copyConditions != null && rules.copyConditions.equalsIgnoreCase("Y")) {
...
function updateOwnerFromGIS(ownerScriptModel, copyGISData, mappingIdField, parcelNumber) {

	if (copyGISData == null || copyGISData == "") {
		return;
	}

	var newValuesMap = getNewAttributesMap(copyGISData, mappingIdField, parcelNumber);
