...
Description : JSON Example :
{
	"Building/Commercial/New Construction/NA": {
		"ApplicationSubmitAfter":[{
			...
			"recordStatus": "Waiting For Revisions",
...
try {
	...
	for (s in settingsArray) {
		...
		if (isJsonRulesMatchRecordData(rules) && isCapStatusMatchRules(rules.recordStatus))
...
function ActivityAutomation(rules) {
	...
	activityModel.setActivityName(rules.activityName);
	if (rules.activityDescription != null && rules.activityDescription != "")
		activityModel.setActivityDescription(rules.activityDescription);
	if (rules.activityDue != null && rules.activityDue != "")
		activityModel.setActDueDate(getFormattedDate(rules.activityDue));
	if (rules.activityUser != null && rules.activityUser != "") {
		var user = aa.person.getUser(rules.activityUser).getOutput();
		activityModel.setAssignedStaffID(rules.activityUser);
	}
	if (rules.activityDept != null && rules.activityDept != "")
		activityModel.setAssignedDeptNumber(rules.activityDept);

	if (rules.activityPriority != null && rules.activityPriority != "")
		activityModel.setPriority(rules.activityPriority);
	activityModel.setActivityType(rules.activityType);
	activityModel.setActStatus(rules.activityStatus);
	activityModel.setStatus(rules.activityStatus);
	activityModel.setActDate(getFormattedDate(rules.activityStart));
