...
function RecordParcelAutomation(rules) {
	...
	if (isEmptyOrNull(recordParcelAttributes)) {
	...
	if (!isEmptyOrNull(rules.updateReferenceParcel)) {
	...
	if (!isEmptyOrNull(rules.copyConditions)) {
	...
	if (!isEmptyOrNull(rules.updateOwnerData)) {
